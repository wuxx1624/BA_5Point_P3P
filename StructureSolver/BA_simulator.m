...
addMorePoseIdx = find(sum(PoseGraphMatrix,2) <= 1);
...
% TODO: I need an index handling to just only fill in the non-sparse part
% of my matrix Jp and Jf.
Jpose_Jfeat_res = cell(size(PoseGraphMatrix,1), 3);
for featk = 1:size(PoseGraphMatrix,1)       
   % a. Extract poses that see this feature
   poses_see_featk = find(PoseGraphMatrix(featk, :));
   
   % b. Allocate Jacobians for poses and features
   Jpose_Jfeat_res{featk, 1} = zeros(2, 6*length(poses_see_featk));   % poses
   Jpose_Jfeat_res{featk, 2} = zeros(2*length(poses_see_featk), 3);   % feat position
   Jpose_Jfeat_res{featk, 3} = zeros(2*length(poses_see_featk), 1);   % feat position
   
   % c. Computation
   for l = 0:length(poses_see_featk)-1
      %  Pose Jacobian
      Cr_p_f = FeatureBag(1:3 ,featk);      
      Cl_z_f = featureExtracted{poses_see_featk(l+1)}(:,PoseGraphMatrix(featk,poses_see_featk(l+1)));                  
      
      
      % Pose Transformation
      Cl_T_W = AbsolutePoses(:,:,poses_see_featk(l+1));
      Cr_T_W = AbsolutePoses(:,:,FeatureBag(4, featk));      
      Cl_T_Cr = Cl_T_W*[InversePose(Cr_T_W);zeros(1,3) 1];            
      Cl_p_f = Cl_T_Cr*[Cr_p_f;1];
      
      
      [Jpose_Jfeat_res{featk, 1}(:,6*l+1:6*l+6), Jpose_Jfeat_res{featk, 3}(2*l+1:2*l+2)] = ...
          dprojection_dp_dtt2(Cr_p_f, Cl_T_Cr(:,1:3), Cl_T_Cr(:,4), Cl_z_f);
                        
      % Feat Jacobian
      Jpose_Jfeat_res{featk, 2}(2*l+1:2*l+2,:) = 1/Cl_p_f(3)*[1 0 -Cl_p_f(1)/Cl_p_f(3);0 1 -Cl_p_f(2)/Cl_p_f(3)] * Cl_T_Cr(:,1:3);
   end   
end
...
for featk = 0:size(PoseGraphMatrix,1)-1
   ...
   for l = 0:length(poses_see_featk)-1
       Jpose(row_count+1:row_count+2, 6*(poses_see_featk(l+1)-1)+1:6*(poses_see_featk(l+1)-1)+6) = ...
           Jpose_Jfeat_res{featk+1, 1}(:,6*l+1:6*l+6);   
       row_count = row_count + 2;
   end
   
   Jfeat(last_row_count+1:row_count, 3*featk+1:3*featk+3) = Jpose_Jfeat_res{featk+1,2};
