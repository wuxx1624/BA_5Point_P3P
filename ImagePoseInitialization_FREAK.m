...
if reloadImages == 1
    Datapath = '/home/jahid/Fei/FREAK/Data';
    featurentotal = cell(NumberOfImages, 1);
    ...
    for i = 1:NumberOfImages
        ...
        featurentotal{i} = load([Datapath,'/',num2str(meaturePoses(i)),'.txt']);
        featureExtracted{i} = featurentotal{i}(:,1:2)';
        featureDescriptor{i} = featurentotal{i}(:,3:end)';
...
for i = 1:NumberOfImages-1
    for j = i+1:NumberOfImages       
        ...
        homo_2 = StaticUndistort(fc,cc,kc,PairWiseMatches{SUTrilInd(NumberOfImages,i,j)}(:,4:5)');
        PairWiseMatches_homo{SUTrilInd(NumberOfImages,i,j)}(:,1:2) = homo_1';
        PairWiseMatches_homo{SUTrilInd(NumberOfImages,i,j)}(:,4:5) = homo_2';
